...
def calculate_user_risk_tolerance(csv_filename):
    """
    Calculate user risk tolerance based on stock transactions.

    Parameters:
    - csv_filename (str): The path to the CSV file containing stock transactions.

    Returns:
    - str: The user's risk tolerance ('low', 'medium', 'high').
    """
    # Load the CSV file
    ...
    avg_quantity = stock_data['Quantity'].mean()

    # Calculate the total number of transactions
    total_transactions = len(stock_data)

    # Calculate the percentage of Buy vs Sell transactions
    buy_transactions = len(stock_data[stock_data['Transaction Type'] == 'Buy'])
    sell_transactions = len(stock_data[stock_data['Transaction Type'] == 'Sell'])
    buy_ratio = buy_transactions / total_transactions

    # Determine risk tolerance based on the calculated metrics
    if avg_price < 100 and avg_quantity < 500 and buy_ratio > 0.7:
        return 'low'
    elif avg_price >= 100 and avg_quantity >= 500 and buy_ratio <= 0.7:
        return 'high'
    else:
        return 'medium'

# Example usage
csv_filename = r"D:\DOT SLASH\stock_transactions.csv"
user_risk_tolerance = calculate_user_risk_tolerance(csv_filename)
